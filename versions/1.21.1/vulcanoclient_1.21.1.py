...
def run_task_with_progress(task_func, *args, **kwargs):
    ...
    queue = Queue()
    thread = threading.Thread(target=run_in_thread, args=(queue,))
    thread.daemon = True
    thread.start()
    ...
    def check_queue():
        try:
            ...
            if status == 'error':
                messagebox.showerror("Fout", f"Er is een fout opgetreden: {result}")
            return
        except:
...
def java_install():
    ...
    if __name__ == "__main__":
        if platform.system() == "Darwin":  # Check voor macOS
...
def vulcanoclient_1_21_1_lunar_installer():
    try:
        ...
        # Controleer en verwijder de lunar_del bestanden als ze bestaan
...
def install_winget_as_admin():
    ...
    def install_winget():
        try:
            # Maak het voortgangsvenster aan
...
def create_gui():
    ...
    # Alleen laadanimatie tonen op Windows
    if platform.system() == "Windows":
...
def run_task_with_progress(task_func, *args, **kwargs):
    ...
    # Controleer periodiek de queue voor resultaten
    def check_queue():
        try:
            status, result = queue.get_nowait()
            if status == 'error':
...
def install_winget_threaded():
    run_task_with_progress(install_winget_as_admin)

def install_lunar_client_threaded():
    run_task_with_progress(install_lunar)

def install_vulcanoclient_lunar_threaded():
    run_task_with_progress(vulcanoclient_1_21_1_lunar_installer)
