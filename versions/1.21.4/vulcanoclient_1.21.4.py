...
URL_CLIENT = "https://github.com/VulcanoSoftware/vulcanoclient/releases/download/1.6/vulcanoclient_1.21.4.zip"
...
def show_tlauncher_instructions():
    ...
    steps = [
        ...
        ("3. Selecteer bij versies Fabric 1.21.4\n", IMAGE_URLS.get("step3")),
...
def show_lunar_instructions():
    ...
    steps = [
        ...
        ("6. Verander hier de versie naar 1.21.4\n", IMAGE_URLS.get("step5_lunar")),
...
def show_minecraft_instructions():
    ...
    steps = [
        ...
        ("2. Selecteer bij versies Fabric 1.21.4\n", IMAGE_URLS.get("step2_mc")),
...
def ask_for_vulcano_client():
    vulcano_response = messagebox.askyesno("VulcanoClient Installatie", "Heb je VulcanoClient al geïnstalleerd?")
    if not vulcano_response:
        install_vulcanoclient()
    else:
        messagebox.showinfo("Informatie", "VulcanoClient is al geïnstalleerd. Volg de instructies om verder te gaan.")
...
def install_fabric(launcher):
    ...
    try:
        command = ["java", "-jar", installer_path, "client", "-dir", minecraft_dir, "-mcversion", "1.21.4"]
...
def vulcanoclient_1_21_4_lunar_installer():
    try:
        ...
        lunar_profiles_lunar_1_21_mods_fabric_1_21_4_path = os.path.join(lunar_profiles_lunar_1_21_mods_path, "fabric-1.21.4")
        ...
        if not os.path.exists(lunar_profiles_lunar_1_21_mods_fabric_1_21_4_path):
            os.makedirs(lunar_profiles_lunar_1_21_mods_fabric_1_21_4_path)
            log_message(f"De map '{lunar_profiles_lunar_1_21_mods_fabric_1_21_4_path}' is aangemaakt.")
        else:
            log_message(f"De map '{lunar_profiles_lunar_1_21_mods_fabric_1_21_4_path}' bestaat al.")
        ...
        lunar_del8 = lunar_profiles_lunar_1_21_mods_fabric_1_21_4_path
        ...
        fabric_source = os.path.join(temp_extract_path, "fabric-1.21.4")
        game_source = os.path.join(temp_extract_path, "game")

        if os.path.exists(fabric_source):
            shutil.copytree(fabric_source, lunar_profiles_lunar_1_21_mods_fabric_1_21_4_path, dirs_exist_ok=True)
            log_message("fabric-1.21.4 map is verplaatst")
...
def lunar_client():
    ...
    if response:
        lunar_prima()
        # Eerst standaardclient installeren
        ask_for_vulcano_client()
        
        kill_lunar_client()
        vulcanoclient_1_21_4_lunar_installer()
...
def create_gui():
    ...
    version_label = tk.Label(root, text="1.21.4", font=("Arial", 10))
